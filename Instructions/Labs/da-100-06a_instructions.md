...
14. **홈** 리본 탭의 **계산** 그룹 내부에서 **새 테이블**을 클릭합니다.

	![그림 5](Linked_image_Files/PowerBI_Lab06A_image7.png)

15. 수식 표시줄에서 다음을 입력합니다.
...
17. 왼쪽 아래 모서리에 있는 상태 표시줄에서 테이블 통계를 보면 데이터 행 1826개가 생성된 것을 알 수 있으며, 이는 5년 전체 데이터에 해당합니다.
...
19. 수식 표시줄에서 다음을 입력한 후 **Enter** 키를 누릅니다.
...
20. 새 열이 추가되었는지 확인합니다.

	![그림 12](Linked_image_Files/PowerBI_Lab06A_image12.png)

21. 조각 파일 정의를 사용하여 **Date** 테이블에 대한 다음과 같은 계산된 열 두 개를 만듭니다.
...
22. 계산의 유효성을 검사하려면 보고서 보기로 전환합니다.

23. 새 보고서 페이지를 만들려면 왼쪽 하단에 있는 더하기 아이콘을 클릭합니다.

	![그림 15](Linked_image_Files/PowerBI_Lab06A_image14.png)

24. 새 보고서 페이지에 행렬 시각적 개체를 추가하려면 **시각화** 창에서 행렬 시각적 유형을 선택합니다. 

	*팁: 각 아이콘 위에 커서를 마우스를 가져가 시각적 유형을 설명하는 도구 설명을 표시할 수 있습니다*.

	![그림 16](Linked_image_Files/PowerBI_Lab06A_image15.png)

25. **필드** 창의 **Date** 테이블 내부에서 **Year** 필드를 **행** 웰로 끌어갑니다.

	![그림 17](Linked_image_Files/PowerBI_Lab06A_image16.png)

26. **Month** 필드를 **Year** 필드 바로 아래 **행** 웰로 끌어갑니다.

	![그림 18](Linked_image_Files/PowerBI_Lab06A_image17.png)

27. 행렬 시각적 개체의 오른쪽 위에서 이중으로 갈라진 화살표 아이콘(모든 연도를 한 수준 아래로 확장)을 클릭합니다.

	![그림 19](Linked_image_Files/PowerBI_Lab06A_image18.png)

28. 연도가 월로 확장되고 월이 연대순이 아닌 영문자순으로 정렬됩니다.
...
29. **월** 필드 정렬 순서를 사용자 지정하려면 데이터 보기로 전환합니다. 

30. **MonthKey** 열을 **Date** 테이블에 추가합니다.
...
31. 데이터 보기에서 새 열에 숫자 값(예: 2017년 7월은 201707)이 포함되어 있는지 확인합니다.

	![그림 21](Linked_image_Files/PowerBI_Lab06A_image20.png)

32. 보고서 보기로 다시 전환합니다.

33. **필드** 창에서 **월** 필드가 선택되었는지 확인합니다(선택하고 나면 배경이 진한 회색이 됨).

34. **열 도구** 상황에 맞는 리본에 있는 **정렬** 그룹에서 **열 기준 정렬**을 클릭한 다음 **MonthKey**를 선택합니다.

	![그림 22](Linked_image_Files/PowerBI_Lab06A_image21.png)

35. 행렬 시각적 개체에서 월이 이제 연대순으로 정렬됩니다.
...
37. **Date** 테이블에서 **MonthKey** 열을 숨깁니다.

38. **Date** 테이블에서 다음 세 가지 수준을 사용하여 **Fiscal**이라는 계층 구조를 만듭니다.
...
42. **필드** 창에서 **날짜** 테이블(필드 아님)을 선택합니다.

43. **테이블 도구** 상황에 맞는 리본에 있는**일정** 그룹 내부에서 **날짜 테이블로 표시**를 클릭하고 **날짜 테이블로 표시**를 선택합니다.

	![그림 8](Linked_image_Files/PowerBI_Lab06A_image24.png)

44. **날짜 테이블로 표시** 창의 **날짜 열** 드롭다운 목록에서 **날짜**를 선택합니다.
...
48. 시각적 필드 창(**시각화** 창 아래에 있음)의 **값** 웰에 **단가**가 나열됩니다.

	![그림 28](Linked_image_Files/PowerBI_Lab06A_image28.png)

49. **Unit Price** 드롭다운 화살표를 클릭하고 나서 사용 가능한 메뉴 옵션을 확인합니다.
...
50. 측정값을 만들려면 **필드** 창에서 **Sales** 테이블을 오른쪽 단추로 클릭한 다음 **새 측정값**을 선택합니다.

	![그림 31](Linked_image_Files/PowerBI_Lab06A_image30.png)

51. 수식 표시줄에서 다음 측정값 정의를 추가합니다.
...
52. **Avg Price** 측정값을 행렬 시각적 개체에 추가합니다.

53. **Unit Price** 열과 동일한 결과(다른 서식 사용)가 생성됩니다.

54. **값** 웰에서 **Avg Price** 필드에 대한 상황에 맞는 메뉴를 열면 집계 기술을 변경할 수 없다는 것을 알게 됩니다.

	![그림 32](Linked_image_Files/PowerBI_Lab06A_image31.png)

55. 코드 조각 파일 정의를 사용하여 **판매** 테이블에 대한 다음의 다섯 가지 측정값을 만듭니다.
...
56. 모델 보기로 전환한 다음 네 가지 가격 측정값, 즉 **Avg Price**, **Max Price**, **Median Price** 및 **Min Price**를 다중 선택합니다.

57. 측정값을 다중 선택하기 위해 다음 요구 사항을 구성합니다.
...
59. **Orders** 및 **Order Lines** 측정값을 다중 선택하고 다음 요구 사항을 구성합니다.
...
60. 보고서 뷰의 행렬 시각적 개체의 **값**에서 **단가** 필드에 대해 X 를 클릭하여 제거합니다.

	![그림 38](Linked_image_Files/PowerBI_Lab06A_image34.png)

61. 행렬 시각적 개체의 크기를 늘려서 페이지 너비와 높이를 채웁니다.

62. 다음의 다섯 가지 새 측정값을 행렬 시각적 개체에 추가합니다.
...
63. 결과가 합리적으로 보이고 서식이 올바르게 지정되었는지 확인합니다.
...
65. 테이블 시각적 개체를 검토하며 **Target** 열의 합계를 확인합니다.
...
66. 테이블 시각적 개체에서 **Target** 필드를 제거합니다. 

	![그림 42](Linked_image_Files/PowerBI_Lab06A_image38.png)

67. **Targets**의 이름을 바꿉니다.** | Target** 열의 이름을 **Targets | TargetAmount**로 바꿉니다.
...
69. 소수점 첫째자리가 되도록 **대상** 측정값의 형식을 지정합니다.

	*팁: **측정값 도구** 상황에 맞는 리본을 사용할 수 있습니다*.

70. **TargetAmount** 열을 숨깁니다.

71. **Target** 측정값을 테이블 시각적 개체에 추가합니다.

72. **Target** 열 합계가 이제 BLANK가 된 것을 확인할 수 있습니다.

	![그림 43](Linked_image_Files/PowerBI_Lab06A_image39.png)

73. 코드 조각 파일 정의를 사용하여 **Targets** 테이블에 대한 다음 두 가지 측정값을 만듭니다.

	* Variance

	* Variance Margin

74. 소수점 0자리가 되도록 **Variance** 측정값의 형식을 지정합니다.

75. **Variance Margin** 측정값을 소수점 두 자리의 백분율로 지정합니다.

76. **Variance** 및 **Variance Margin** 측정값을 테이블 시각적 개체에 추가합니다.

77. 테이블 시각적 개체를 넓혀 모든 값이 표시되도록 합니다.
