...
1. 다음 식에 따라 **Sales** 테이블에 측정값을 추가합니다.
	...
	*이 수식에서 측정값은 수정된 필터 컨텍스트에서 **Sales** 열의 합을 평가하여 **Region** 테이블에 적용되는 필터를 제거합니다*.
...
3. **Sales All Region** 측정값은 각 지역, 국가(소계) 및 그룹(소계)에 대한 전 지역 매출의 합계를 계산합니다.
	...
	*이 측정값은 아직 유용한 결과를 제공하지 않습니다. 그룹, 국가 또는 지역의 매출을 이 값으로 나눌 때 "총계의 백분율"이라고 하는 유용한 비율을 생성합니다*.
...
4. **필드** 창에서 **Sales All Region** 측정값이 선택되었는지 확인하고 수식 표시줄에서 측정값 이름과 수식을 다음 수식으로 바꿉니다.
	...
	**DAX**
	...
	```   
	*이 측정값은 업데이트된 수식을 정확히 반영하도록 이름이 변경되었습니다. **DIVIDE()** 함수는 **Region** 테이블에 적용된 필터를 제거하는 수정된 컨텍스트에서 **영업** 측정값으로 **영업** 측정값(필터 컨텍스트에 의해 수정되지 않음)을 분할합니다*.
...
9.  **Sales % Country** 측정값 수식은 **Sales % All Region** 측정값 수식과는 조금 다릅니다.
	...
	*차이점은 분모가 **Region** 테이블의 모든 열이 아닌 **Region** 테이블의 **Region** 열에 대해 필터를 제거하여 필터 컨텍스트를 수정한다는 것입니다. 즉, 그룹 또는 국가 열에 적용되는 모든 필터가 유지됩니다. 국가의 백분율로 매출을 나타내는 결과를 얻습니다*.
...
14. 다음 식을 기반으로 **Sales** 테이블에 다른 측정값을 추가하고 백분율로 형식을 지정합니다.
	...
	*그룹의 백분율로 매출을 달성하기 위해 두 개의 필터를 적용하여 두 열의 필터를 효과적으로 제거할 수 있습니다*.
...
이 연습에서는 매출 YTD(연간 누계) 측정값과 매출 YoY(전년대비) 성장률 측정값을 만듭니다.
...
4. 해당 연도의 누적 매출 값을 확인합니다.
	...
	*TOTALYTD() 함수는 필터 조작, 특히 시간 필터 조작을 수행합니다. 예를 들어, 2017년 9월(회계 연도의 세 번째 달)에 대한 YTD 매출을 계산하려면 **Date** 테이블의 모든 필터가 제거되고 연초에 시작하는 새 날짜 필터(2017년 7월 1일)로 대체되며 컨텍스트 내 날짜 기간의 마지막 날짜(2017년 9월 30일)까지 연장됩니다.*
