...
18. **(공백)**의 응답을 확인합니다.
	...
	***랩 06B**에서 **판매 YTD** 측정값을 추가했던 것을 기억해 주세요. 이 측정값은 시간 인텔리전스 식이며 결과를 생성하려면 **날짜** 테이블의 필터가 필요합니다*.
...
25. **타일 세부 정보** 창(오른쪽에 위치)에서 **부제** 상자에 **회계연도 2020**을 입력합니다.
...
27. **판매 YTD** 타일에 부제가 표시됩니다.

	![그림 21](Linked_image_Files/PowerBI_Lab10A_image19.png)

28. **판매, 이익률** 타일에 대한 타일 세부 정보를 편집합니다.

29. **타일 세부 정보** 창의 **기능** 섹션에서 **마지막 새로 고침 시간 표시**를 확인합니다.
...
31. 타일은 마지막 새로 고침 시간(Power BI Desktop에서 데이터 모델을 새로 고친 시간)을 설명합니다.
...
38. **D:\DA100\Setup** 폴더 내부에서 마우스 오른쪽 버튼으로 **UpdateDatabase-3-AddPartnerAccount.ps1** 파일을 클릭한 다음 **PowerShell에서 실행**을 선택합니다.

39. 메시지가 표시되면 강의실 파트너의 계정 이름을 입력한 다음 **Enter** 키를 누릅니다.
...
41. **쿼리** 그룹 내부의 **홈** 리본에서 **새로 고침**을 클릭합니다.

	![그림 30](Linked_image_Files/PowerBI_Lab10A_image27.png)

42. 새로 고침이 완료되면 Power BI Desktop 파일을 저장합니다.

43. 파일을 **판매 분석** 작업 영역에 게시합니다.

44. 데이터 세트를 바꾸라는 메시지가 표시되면 **바꾸기**를 클릭합니다.

	![그림 31](Linked_image_Files/PowerBI_Lab10A_image28.png)

	*이제 Power BI 서비스의 데이터 세트에는 2020년 6월 판매 데이터가 있습니다*.

45. Power BI Desktop을 닫습니다.

46. Edge에서 **판매 분석** 작업 영역의 Power BI 서비스에 **판매 분석** 보고서도 게시된 것을 확인합니다.

	이 보고서는 **랩 05A** 및 **랩 06A**에서 개발한 모델을 테스트하는 데 사용되었습니다.

47. 데이터 세트가 아닌 **판매 분석** 보고서를 제거합니다.
...
49. **이제** **판매, 이익 마진** 타일의 자막에서 데이터가 새로 고쳐졌습니다.

50. 또한 **2020년 6월**에 대한 열이 있음을 알 수 있습니다.

	![그림 33](Linked_image_Files/PowerBI_Lab10A_image29.png)

	***판매 YTD** 타일에 대한 경고도 트리거되어야 합니다. 잠시 후 경고는 판매가 구성된 임계값을 초과한다는 것을 알려 줍니다*.

51. **판매 YTD** 타일이 **$37M**으로 업데이트되었음을 알 수 있습니다.

52. **판매 YTD** 타일에 경고 알림 아이콘이 표시되는지 확인합니다.
